...
class FernScribeGitHub {
  constructor() {
    ...
    // Detect event type and set appropriate context
    this.eventType = process.env.EVENT_TYPE || 'issues';
    ...
    this.prBaseRef = process.env.PR_BASE_REF;
    this.prHeadRef = process.env.PR_HEAD_REF;
  ...
  extractTextFromDiff(patch) {
    if (!patch) return '';
    
    // Extract added lines (lines starting with +) and remove the + prefix
  ...
  async extractChangedContent() {
    console.log('📄 Extracting changed content from PR...');
    const files = await this.getPRChanges();
    
    const changedContent = [];
    
    for (const file of files) {
      // Focus on documentation files (md, mdx, etc.)
      if (!file.filename.match(/\.(md|mdx|txt|rst)$/i)) {
        continue;
      }
      ...
      const addedText = this.extractTextFromDiff(file.patch);
    ...
    console.log(`   📊 Found ${changedContent.length} files with meaningful text changes`);
    return changedContent;
  }

  async findConflictingContent(changedContent) {
    console.log('🔍 Searching for potentially conflicting content...');
    
    const conflicts = [];
    
    for (const change of changedContent) {
      console.log(`   🔍 Analyzing changes in: ${change.filename}`);
      
      // Extract key phrases that might conflict (like "newest", "latest", "best", etc.)
      const conflictKeywords = this.extractConflictKeywords(change.addedText);
      
      if (conflictKeywords.length === 0) {
        continue;
      }

      console.log(`   🎯 Found potential conflict keywords: ${conflictKeywords.join(', ')}`);
      
      // Search for existing content with similar claims
      for (const keyword of conflictKeywords) {
        const searchResults = await this.queryTurbopuffer(keyword, {
          namespace: process.env.TURBOPUFFER_NAMESPACE || 'default',
          topK: 5,
          urlsToIgnore: [`/${change.filename}`] // Don't match the file we're changing
        });

        // Look for content that might conflict
        for (const result of searchResults) {
          const conflictScore = this.calculateConflictScore(keyword, result.chunk || result.document || '');
          
          if (conflictScore > 0.7) { // Threshold for considering it a conflict
            conflicts.push({
              changeFile: change.filename,
              conflictFile: result.pathname || result.url,
              keyword,
              conflictText: (result.chunk || result.document || '').substring(0, 200) + '...',
              score: conflictScore,
              reason: `Both files mention "${keyword}" which may create conflicting information`
            });
          }
        }
      }
  ...
  extractConflictKeywords(text) {
    // Look for potentially conflicting phrases
    const conflictPatterns = [
      /\b(newest|latest|current|modern|updated|recent)\s+\w+/gi,
      /\b(best|top|leading|premier|primary)\s+\w+/gi,
      /\b(first|only|main|primary|default)\s+\w+/gi,
      /\b(recommended|preferred|suggested)\s+\w+/gi,
      /\bversion\s+[\d.]+/gi,
      /\breleased?\s+(today|yesterday|recently|in\s+\d+)/gi
    ];
  ...
  calculateConflictScore(keyword, content) {
    ...
    // Higher score if the keyword appears in similar context
    const contextWords = ['newest', 'latest', 'best', 'recommended', 'current', 'modern', 'top', 'leading'];
    let contextMatches = 0;
    
    for (const word of contextWords) {
      if (contentLower.includes(word) && keywordLower.includes(word)) {
        contextMatches++;
      }
    }

    return Math.min(1.0, 0.5 + (contextMatches * 0.2));
  }

  async generateConflictRecommendations(conflicts) {
    if (conflicts.length === 0) {
      return null;
    }

    const prompt = `You are analyzing a Pull Request for potential documentation conflicts. Here are the conflicts found:

${conflicts.map((conflict, i) => `
${i + 1}. **Conflict in ${conflict.changeFile}**
   - Conflicting keyword/phrase: "${conflict.keyword}"
   - May conflict with: ${conflict.conflictFile}
   - Existing content: ${conflict.conflictText}
   - Reason: ${conflict.reason}
`).join('\n')}

Please provide specific, actionable recommendations for resolving these conflicts. For each conflict, suggest:
1. Whether it's a real conflict that needs attention
2. What specific changes should be made to resolve it
3. Which files need to be updated

Format your response as a clear markdown comment that can be posted on a GitHub PR.

Start with a brief summary, then list each conflict with specific recommendations.`;
    ...
    try {
      ...
      console.error('Failed to generate conflict recommendations:', error);
      return null;
    }
  }

  async commentOnPR(body) {
    try {
...
**Please review the changes carefully before merging.**`;
  ...
  async processIssue() {
    try {
      ...
      console.log('\n✅ Fern Scribe GitHub issue workflow complete!');

    } catch (error) {
      console.error('❌ Fern Scribe GitHub issue processing failed:', error);
      throw error;
    }
  }
  ...
  async processPR() {
    try {
      console.log('🔀 Processing GitHub pull request...');
      console.log(`📋 PR #${this.prNumber}: ${this.prTitle}`);
      
      // Quick check: Skip draft PRs or bot PRs to save processing
      ...
      if (pr.user?.type === 'Bot') {
        console.log('⏭️  Skipping bot PR - automated changes don\'t need conflict analysis');
        return;
      }
      
      // Extract changed content from the PR
      const changedContent = await this.extractChangedContent();
      
      if (changedContent.length === 0) {
        console.log('ℹ️  No documentation changes found in this PR');
        return;
      }

      console.log(`📊 Found changes in ${changedContent.length} documentation files:`);
      changedContent.forEach(change => {
        console.log(`   📄 ${change.filename} (+${change.additions} lines)`);
      });

      // Find potentially conflicting content
      const conflicts = await this.findConflictingContent(changedContent);
      
      if (conflicts.length === 0) {
        console.log('✅ No potential conflicts found with existing documentation');
        
        // Leave a positive comment
        await this.commentOnPR(`## 🌿 Fern Scribe Analysis

✅ **No Documentation Conflicts Detected**

I've analyzed your documentation changes and found no conflicts with existing content. Your changes look good from a content consistency perspective!

**Files Analyzed:**
${changedContent.map(change => `- \`${change.filename}\` (+${change.additions} lines)`).join('\n')}
...
*Powered by AI and TurboBuffer content analysis*`);
      ...
      conflicts.forEach((conflict, i) => {
        console.log(`   ${i + 1}. ${conflict.changeFile} ↔️ ${conflict.conflictFile}`);
        console.log(`      Keyword: "${conflict.keyword}" (score: ${conflict.score.toFixed(2)})`);
      });

      // Generate AI recommendations for conflicts
      console.log('🤖 Generating recommendations for resolving conflicts...');
      const recommendations = await this.generateConflictRecommendations(conflicts);
      
      if (recommendations) {
        // Comment on the PR with recommendations
        const prComment = `## 🌿 Fern Scribe Analysis
...
${conflicts.map((conflict, i) => `${i + 1}. \`${conflict.changeFile}\` may conflict with \`${conflict.conflictFile}\` (keyword: "${conflict.keyword}")`).join('\n')}

*Powered by AI and TurboBuffer content analysis*`;

        await this.commentOnPR(prComment);
        console.log('✅ Posted conflict analysis and recommendations to PR');
      } else {
        console.log('⚠️  Could not generate recommendations, posting basic conflict info');
        
        const basicComment = `## 🌿 Fern Scribe Analysis

⚠️ **Potential Documentation Conflicts Detected**

I found ${conflicts.length} potential conflicts with existing documentation:

${conflicts.map((conflict, i) => `${i + 1}. **${conflict.changeFile}** may conflict with **${conflict.conflictFile}**
   - Conflicting content: "${conflict.keyword}"
   - ${conflict.reason}`).join('\n\n')}

Please review these conflicts and update the documentation as needed to maintain consistency.

*Powered by AI and TurboBuffer content analysis*`;

        await this.commentOnPR(basicComment);
      }
      
      console.log('\n✅ Fern Scribe GitHub PR workflow complete!');

    } catch (error) {
      console.error('❌ Fern Scribe GitHub PR processing failed:', error);
      throw error;
    }
  }
...
module.exports = FernScribeGitHub;
